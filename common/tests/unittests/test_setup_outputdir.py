...
class SetupOutputDirTestCase(unittest.TestCase):
    def test_os_path(self):
        ...
        with pytest.raises(Exception):
            path = 2.2
            setup_outputdir(
                path, warn_if_exist=True, create_dir=False, path_suffix=None
            )
        ...
        returned_path = setup_outputdir(
            path, warn_if_exist=True, create_dir=False, path_suffix=path_suffix
        )
    ...
    def test_s3_path(self):
        ...
        with patch("os.makedirs") as mock_makedirs:
            returned_path = setup_outputdir(
                path, warn_if_exist=True, create_dir=True, path_suffix=None
            )
