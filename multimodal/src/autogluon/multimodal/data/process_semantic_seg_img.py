...
class SemanticSegImageProcessor:
            ...
            fn.update(
                {
                    ...
                    self.image_valid_num_key: StackCollator(),
                    self.label_key: PadCollator(pad_val=0),
                    self.mask_label_key: PadCollator(pad_val=0),
                    self.class_label_key: PadCollator(pad_val=0),
                }
            )
    ...
    def process_one_sample(
        ...
        """
        Read images, process them, and stack them binary semantic segmentation. One sample can have multiple images,
    ...
    def process_one_sample_multi_class(
        self,
        image_features: Dict[str, Union[List[str], List[bytearray]]],
        feature_modalities: Dict[str, List[str]],
        is_training: bool,
        ...
        """
        Read images, process them, and stack them for multi-class semantic segmentation. One sample can have multiple images,
        resulting in a tensor of (n, 3, size, size), where n <= max_img_num_per_col is the available image number.
        ...
        for idx, img_feature in enumerate(per_col_image_features[: self.max_img_num_per_col]):
            ...
            if is_training or annotation_column is not None:
                ...
                classes = classes[classes != self.ignore_label].to(torch.int64)
                ...
                if len(masks) == 0:
                    ...
                    gt_masks_per_category.append(torch.zeros((0, gt.shape[-2], gt.shape[-1])))
                else:
                    gt_masks_per_category.append(torch.stack(masks))
        ...
        ret.update(
            {
                ...
                self.image_valid_num_key: len(images),
                self.label_key: torch.cat(gts, dim=0) if len(gts) != 0 else torch.tensor([]),
                self.mask_label_key: torch.cat(gt_masks_per_category, dim=0)
                if len(gt_masks_per_category) != 0
                else torch.tensor([]),
                self.class_label_key: torch.cat(gt_classes, dim=0) if len(gt_classes) != 0 else torch.tensor([]),
            }
        )
        return ret
    ...
    def __call__(
        ...
        if self.num_classes == 1:
            ...
            return self.process_one_sample_multi_class(images, feature_modalities, is_training)
