"""Implementation of the multimodal predictor with learners. 
In connection with the autogluon tabular, this predictor acts as the master coordinator that 
diaptches the correct learner to complete user intention. This is the user-facing API for automl multimodal.
Similar to learners in tabular, learners here handle global parameters and data processing.
Different learners may have different logics in data processing and global parameter settings. 
We leave that up to the learners to implement.
The trainer in tabular context is hereby the pl.Trainer in multimodal, which takes in data, preprocesses it and trains the model.
Trainer (a.k.a. AutoTrainer in tabular) never manipulates any global parameters or data.
"""
...
class MultiModalPredictorWithLearner(ExportMixin):
    """
    ...
    def __init__(
        ...
        # leave this for the learner to handle
        # sanity check of eval_metric
        ...
        # self._warn_if_exist = warn_if_exist  # this is not used
    ...
    def fit(
        ...
        # FIXME: infer output shape for non-detection problems. Should be merged with step 8. NOTE: Can be refactored into Learners
        if self._problem_type != OBJECT_DETECTION:
