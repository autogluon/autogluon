...
class CLIPForImageText_fusionmlp(nn.Module):
    ...
    def __init__(
        ...
        if (
            column_types != None
        ):  # use the column_types to deal with the situation where the data does not has image and text simultaneously
        ...
        if not self.has_image or not self.has_text:
            for k, v in self.model.named_parameters():
                if not self.has_image and ("vision" in k or "visual" in k):
                    v.requires_grad = False
                if not self.has_text and "text" in k:
                    v.requires_grad = False
