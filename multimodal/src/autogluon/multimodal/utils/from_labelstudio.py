...
class LabelStudioReader:
    ...
    def __init__(self,host=None):
        ...
        print("NOTE: the default label-studio host is {},if you want to"\
            "get data from an running label-studio url, please set 'ls_host_on' to True".format(self.default_host))
    ...
    def get_columns_by_type(self,type,format):
        ...
        if type.value != 'customize':
            try:
                ...
                raise ValueError("this format is not supported by current task type.")
        else:
            return [],[]
    ...
    def process_data_str(self,s:str,ls_online):
        ...
        if ls_online:
            if s.startswith("/data/"):
                ...
                # data are accessable when label-studio host is on
                # data can be addressed through a temperal ls-host-based URL  
    ...
    def from_image_classification(self,path,ls_host_on,                        
        ...
        # get file extension with os, distiguish JSON and JSON-MIN
        _,file_extension=os.path.splitext(path)
        file_extension=file_extension[1:]
