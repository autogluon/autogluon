    ...
    if len(checkpoint_paths) > 1:
        ...
        for per_path in checkpoint_paths:
            if os.path.isdir(per_path + "-dir"):  # deepspeed save checkpoints into a directory
                ...
                state_dict = torch.load(per_path, map_location=torch.device("cpu"), weights_only=True)["state_dict"]
            else:
                state_dict = torch.load(per_path, map_location=torch.device("cpu"), weights_only=True)["state_dict"]
        ...
        avg_state_dict = torch.load(checkpoint_paths[0], map_location=torch.device("cpu"), weights_only=True)[
            "state_dict"
        ]
