...
custom_imports = dict(imports=["autogluon.multimodal.models.codetr"], allow_failed_imports=False)
...
model = dict(
    ...
    roi_head=[
        dict(
            ...
            bbox_head=dict(
                ...
                loss_cls=dict(
                    type="CrossEntropyLoss", use_sigmoid=False, loss_weight=1.0 * num_dec_layer * loss_lambda
                ),
    ...
    bbox_head=[
        dict(
            ...
            loss_centerness=dict(
                type="CrossEntropyLoss", use_sigmoid=True, loss_weight=1.0 * num_dec_layer * loss_lambda
            ),
        ),
    ],
    ...
    train_cfg=[
        ...
        dict(
            ...
            rpn_proposal=dict(
                nms_pre=4000, max_per_img=1000, nms=dict(type="nms", iou_threshold=0.7), min_bbox_size=0
            ),
            rcnn=dict(
                assigner=dict(
                    type="MaxIoUAssigner",
...
train_dataloader = dict(
    ...
    dataset=dict(
        pipeline=train_pipeline, dataset=dict(filter_cfg=dict(filter_empty_gt=False), pipeline=load_pipeline)
    ),
)
