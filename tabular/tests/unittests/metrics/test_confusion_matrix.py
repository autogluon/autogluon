...
def test_confusion_matrix_basic(tmp_path):
    ...
    predictor = TabularPredictor(label="label", path=tmp_path).fit(train_data, presets="medium_quality")
...
def test_confusion_matrix_normalize(tmp_path):
    ...
    predictor = TabularPredictor(label="label", path=tmp_path).fit(train_data)
...
def test_confusion_matrix_invalid_problem_type(tmp_path):
    ...
    predictor = TabularPredictor(label="label", path=tmp_path, problem_type="regression").fit(train_data)
...
def test_confusion_matrix_custom_labels(tmp_path):
    ...
    predictor = TabularPredictor(label="label", path=tmp_path, problem_type="binary").fit(
...
def test_confusion_matrix_arguments(tmp_path):
    ...
    predictor = TabularPredictor(label="label", path=str(tmp_path)).fit(train_data, presets="medium_quality")
...
def test_confusion_matrix_bagged_oof(tmp_path):
    ...
    predictor = TabularPredictor(label="label", path=str(tmp_path), problem_type="binary").fit(
        train_data, num_bag_folds=2, presets="medium_quality"
    )
