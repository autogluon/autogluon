...
class ConfigBuilder:
    ...
    def with_presets(self, presets):
        valid_keys = list(tabular_presets_dict.keys())
        if not isinstance(presets, list):
            presets = [presets]
        for preset in presets:
            assert preset in valid_keys, f'{preset} is not one of the valid presets {valid_keys}'
        self.config['presets'] = presets
        return self

    def with_excluded_model_types(self, models):
    ...
    def with_included_model_types(self, models):
        valid_keys = [m for m in MODEL_TYPES.keys() if m not in ['ENS_WEIGHTED', 'SIMPLE_ENS_WEIGHTED']]
        if not isinstance(models, list):
            models = [models]
        for model in models:
    ...
    def with_auto_stack(self, auto_stack=True):
        self.config['auto_stack'] = auto_stack
        return self

    def with_use_bag_holdout(self, use_bag_holdout=True):
        self.config['use_bag_holdout'] = use_bag_holdout
        return self

    def with_num_bag_folds(self, num_bag_folds):
        self.config['num_bag_folds'] = num_bag_folds
        return self

    def with_num_bag_sets(self, num_bag_sets):
        self.config['num_bag_sets'] = num_bag_sets
        return self

    def with_num_stack_levels(self, num_stack_levels):
        self.config['num_stack_levels'] = num_stack_levels
        return self

    def with_holdout_frac(self, holdout_frac):
