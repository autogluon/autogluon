...
class LGBModel(AbstractModel):
    ...
    def _fit(self, X=None, y_train=None, X_val=None, y_val=None, dataset_train=None, dataset_val=None, time_limit=None, num_gpus=0, sample_weight=None, sample_weight_val=None, **kwargs):
        ...
        dataset_train, dataset_val = self.generate_datasets(X=X, y_train=y_train, params=params, X_val=X_val, y_val=y_val,
    ...
    def generate_datasets(self, X: DataFrame, y_train: Series, params, X_val=None, y_val=None, sample_weight=None, sample_weight_val=None, dataset_train=None, dataset_val=None, save=False):
        ...
        y_val_og = None
        if self.problem_type == SOFTCLASS:
            if (not dataset_train) and (X is not None) and (y_train is not None):
                y_train_og = np.array(y_train)
                y_train = pd.Series([0]*len(X))  # placeholder dummy labels to satisfy lgb.Dataset constructor
        ...
        if not dataset_train:
            ...
            dataset_train = construct_dataset(x=X, y=y_train, location=f'{self.path}datasets{os.path.sep}train', params=data_params, save=save, weight=sample_weight)
            # dataset_train = construct_dataset_lowest_memory(X=X, y=y_train, location=self.path + 'datasets/train', params=data_params)
        if (not dataset_val) and (X_val is not None) and (y_val is not None):
    ...
    def _hyperparameter_tune(self, X, y_train, X_val, y_val, scheduler_options, **kwargs):
        ...
        warnings.filterwarnings('ignore', message='Overriding the parameters from Reference Dataset.')
        warnings.filterwarnings('ignore', message='categorical_column in param dict is overridden.')
        dataset_train, dataset_val = self.generate_datasets(X=X, y_train=y_train, params=params_copy, X_val=X_val, y_val=y_val)
