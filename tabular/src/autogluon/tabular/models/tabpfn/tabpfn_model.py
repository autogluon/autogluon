    ...
    def _fit(self, X: pd.DataFrame, y: pd.Series, **kwargs):
        ...
        if max_classes is not None and self.num_classes > max_classes:
            ...
            raise AssertionError(
                f"Max allowed classes for the model is {max_classes}, " f"but found {self.num_classes} classes."
            )
        ...
        if max_features is not None and num_features > max_features:
            raise AssertionError(
                f"Max allowed features for the model is {max_features}, " f"but found {num_features} features."
            )
