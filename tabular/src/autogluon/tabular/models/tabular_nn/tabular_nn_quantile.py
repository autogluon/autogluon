...
class TabularQuantileAggregatorModel(TabularNeuralQuantileModel):
    """
    Class for neural network models that operate on tabular data to aggregate quantile predictions.
    """
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def _fit(self, X, y, X_val=None, y_val=None,
        ...
        elif self.base_pred_features is None:
            self.base_pred_features = self.feature_metadata.type_group_map_special['stack']
        if num_cpus is not None:
            self.num_dataloading_workers = max(1, int(num_cpus/2.0))
        else:
            self.num_dataloading_workers = 1
        ...
        train_dataset, val_dataset = self.generate_datasets(X=X, y=y, params=params, X_val=X_val, y_val=y_val)
        logger.log(15, "Training data for TabularQuantileAggregatorModel has: %d examples, %d features "
                       "(%d vector, %d embedding, %d language)" %
                   (train_dataset.num_examples, train_dataset.num_features,
                    len(train_dataset.feature_groups['vector']), len(train_dataset.feature_groups['embed']),
                    len(train_dataset.feature_groups['language'])))
        ...
        if num_gpus is not None and num_gpus >= 1:
            if torch.cuda.is_available():
                self.device = torch.device("cuda")
                if num_gpus > 1:
                    logger.warning("TabularQuantileAggregatorModel not yet configured to use more than 1 GPU."
                                   " 'num_gpus' set to >1, but we will be using only 1 GPU.")
            else:
                self.device = torch.device("cpu")
        else:
    ...
    def get_net(self, train_dataset, params):
        from .tabular_nn_torch import NeuralQuantileAggregator

        # set network params
        self.model = NeuralQuantileAggregator(num_base_preds=train_dataset.num_base_preds,
    ...
    def _predict_proba(self, X, **kwargs):
        ...
        from .tabular_nn_torch import TabularPyTorchDataset
        if isinstance(X, TabularPyTorchDataset):
            return self._predict_tabular_data(new_orig_data=X, new_pred_data=None, process=False)
        elif isinstance(X, pd.DataFrame):
            pred_Y = X[self.base_pred_features]
            orig_X = X.drop(columns=self.base_pred_features)
            orig_X = self.preprocess(orig_X, **kwargs)
            return self._predict_tabular_data(new_orig_data=orig_X, new_pred_data=pred_Y, process=True)
        else:
            raise ValueError("X must be of type pd.DataFrame or TabularPyTorchDataset, not type: %s" % type(X))

    def _predict_tabular_data(self, new_orig_data, new_pred_data, process=True, predict_proba=True):
        from .tabular_nn_torch import TabularPyTorchDataset
        if process:
            new_data = self.process_test_data(new_orig_data, new_pred_data, None)
        else:
    ...
    def generate_datasets(self, X, y, params, X_val=None, y_val=None):
        ...
        if isinstance(X, TabularPyTorchDataset):
            ...
            train_dataset = self.process_train_data(orig_df=orig_X, pred_df=pred_Y, labels=y,
                                                    impute_strategy=impute_strategy,
                                                    max_category_levels=max_category_levels,
                                                    skew_threshold=skew_threshold,
                                                    embed_min_categories=embed_min_categories,
        ...
        if X_val is not None:
            if isinstance(X_val, TabularPyTorchDataset):
                ...
                val_dataset = self.process_test_data(orig_df=orig_X_val, pred_df=pred_Y_val, labels=y_val)
        else:
            val_dataset = None
        return train_dataset, val_dataset

    def process_test_data(self, orig_df, pred_df, labels=None, **kwargs):
        """ Process train or test DataFrame into a form fit for neural network models.
            Args:
                orig_df (pd.DataFrame): Data to be processed (original feature X)
                pred_df (pd.DataFrame): Data to be aggregated (base model predictions y')
                labels (pd.Series): labels to be processed (y)
            Returns:
                Dataset object
        """
        ...
        return TabularPyTorchDataset(orig_df, self.feature_arraycol_map, self.feature_type_map, labels, pred_df)

    def process_train_data(self, orig_df, pred_df, impute_strategy, max_category_levels, skew_threshold,
                           embed_min_categories, use_ngram_features, labels, **kwargs):
        from .tabular_nn_torch import TabularPyTorchDataset

        # sklearn processing n_quantiles warning
        warnings.filterwarnings("ignore", module='sklearn.preprocessing')
        if set(orig_df.columns) != set(self.features):
        ...
        self._types_of_features, orig_df = self._get_types_of_features(orig_df, skew_threshold=skew_threshold,
                                                                       embed_min_categories=embed_min_categories,
                                                                       use_ngram_features=use_ngram_features)
        logger.log(15, "AutoGluon Quantile Aggregator(pytorch) infers features are of the following types:")
        logger.log(15, json.dumps(self._types_of_features, indent=4))
        logger.log(15, "\n")
        self.processor = self._create_preprocessor(impute_strategy=impute_strategy,
                                                   max_category_levels=max_category_levels)
    ...
    def load(cls, path: str, reset_paths=True, verbose=True):
        ...
        if model._architecture_desc is not None:
            ...
            # recreate network from architecture description
            model.model = NeuralQuantileAggregator(num_base_preds=len(model.base_pred_features),
