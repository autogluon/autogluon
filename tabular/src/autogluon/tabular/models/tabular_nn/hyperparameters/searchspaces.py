...
def get_default_searchspace(problem_type, framework, num_classes=None):
    ...
    mxnet_params = {
        ...
        'layers': ag.space.Categorical(None, [200, 100], [256], [100, 50], [200, 100, 50], [1024], [32], [300, 150]),
        'network_type': ag.space.Categorical('widedeep', 'feedforward'),
        'activation': ag.space.Categorical('relu', 'softrelu'),
        'batch_size': ag.space.Categorical(512, 1024, 2056, 128),
    }
    pytorch_params = {
        'use_batchnorm': ag.space.Categorical(False, True),
...
def get_searchspace_regression(framework):
    params = {
        'weight_decay': ag.space.Real(1e-12, 1.0, default=1e-6, log=True),
    }
    mxnet_params = {
        'activation': ag.space.Categorical('relu', 'softrelu', 'tanh'),
    }
    pytorch_params = {
        'activation': ag.space.Categorical('relu', 'elu', 'tanh'),
    }
...
def get_searchspace_quantile(framework):
    ...
    params = {
        'activation': ag.space.Categorical('relu', 'elu', 'tanh'),
        'weight_decay': ag.space.Real(1e-12, 1.0, default=1e-6, log=True),
        'gamma': ag.space.Real(0.1, 10.0, default=5.0),
        'alpha': ag.space.Categorical(0.001, 0.01, 0.1, 1.0),
    }
    return params
