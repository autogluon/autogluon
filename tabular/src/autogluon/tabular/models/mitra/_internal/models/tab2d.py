...
class Padder(torch.nn.Module):
    ...
    def _init_flash_attn(self, x: torch.Tensor, n_obs: int, n_feat: int):
        ...
        x_of, self.indices_of, self.cu_seqlens_of, self.max_seqlen_in_batch_of = unpad_input(
            x_o, ~self.feature_mask_big
        )

        x_rearranged = einx.rearrange("b s f d -> b f s d", x)
        x_f, self.indices_f, self.cu_seqlens_f, self.max_seqlen_in_batch_f = unpad_input(
        ...
        x_fo, self.indices_fo, self.cu_seqlens_fo, self.max_seqlen_in_batch_fo = unpad_input(
