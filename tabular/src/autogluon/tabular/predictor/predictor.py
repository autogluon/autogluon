    ...
    **kwargs :
    ...
    def fit(
        ...
        **kwargs :
        ...
        train_data, tuning_data, unlabeled_data = self._validate_fit_data(
            train_data=train_data, tuning_data=tuning_data, unlabeled_data=unlabeled_data
        )
        ...
        if holdout_frac is None:
            holdout_frac = default_holdout_frac(
                len(train_data), ag_args.get("hyperparameter_tune_kwargs", None) is not None
            )
    ...
    def _post_fit(
        ...
        if refit_full is not False:
            ...
            if refit_full == "best":
                self.refit_full(
                    model=trainer_model_best, set_best_to_refit_full=_set_best_to_refit_full, **refit_full_kwargs
                )
            else:
    ...
    def transform_features(
        self, data=None, model: str = None, base_models: List[str] = None, return_original_features: bool = True
    ) -> pd.DataFrame:
        """
    ...
    def _get_dataset(data, allow_nan: bool = False):
        if data is None:
            if allow_nan:
                ...
                raise TypeError(
                    "data=None is invalid. data must be a TabularDataset or pandas.DataFrame or str file path to data"
                )
    ...
    def _validate_fit_data(
        ...
        if tuning_data is not None:
            ...
            train_features, tuning_features = self._prune_data_features(
                train_features=train_features, other_features=tuning_features, is_labeled=True
            )
        ...
        if unlabeled_data is not None:
            ...
            train_features, unlabeled_features = self._prune_data_features(
                train_features=train_features, other_features=unlabeled_features, is_labeled=False
            )
