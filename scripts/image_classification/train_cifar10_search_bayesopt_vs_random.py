""" Compares Random search vs Bayesian optimization to select hyperparameters in CIFAR10 image classification task """
import logging
import argparse
import os
...
if __name__ == '__main__':
    ...
    rs_results = task.fit(dataset,
                       ...
                       demo=args.demo)
    logger.info('Best result under random search:')
    logger.info(rs_results.metric)
    logger.info('=========================')
    logger.info('Best config under random search:')
    logger.info(rs_results.config)
    logger.info('=========================')
    logger.info('Time cost under random search:')
    logger.info(rs_results.time)
    logger.info('=========================')
    # BayesOpt Search (via skopt):
    logger.info('Running BayesOpt search (skopt)...')
    bo_results = task.fit(dataset,
                       nets=ag.Nets(net_list),
                       optimizers=ag.Optimizers(optim_list),
                       searcher='bayesopt',
                       trial_scheduler=args.trial_scheduler,
                       ...
                       lr_step=args.lr_step,
                       demo=args.demo)
    logger.info('Best result under BayesOpt search:')
    logger.info(bo_results.metric)
    logger.info('=========================')
    logger.info('Best config under BayesOpt search:')
    logger.info(bo_results.config)
    logger.info('=========================')
    logger.info('Time cost under BayesOpt search:')
    logger.info(bo_results.time)
    logger.info('=========================')
