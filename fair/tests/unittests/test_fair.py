...
def test_metrics():
    ...
    for met, group_met in zip(met_list, group_met_list):
        assert np.isclose(met(A, B), group_met(A, B, C)[0], 1e-5)
...
def test_metrics_identities():
    ...
    assert np.isclose(gm.pos_data_rate(A, B, C), 1 - gm.neg_data_rate(A, B, C)).all()
    assert np.isclose(gm.pos_pred_rate(A, B, C), 1 - gm.neg_pred_rate(A, B, C)).all()
    assert np.isclose(gm.true_pos_rate(A, B, C), 1 - gm.false_neg_rate(A, B, C)).all()
    assert np.isclose(gm.true_neg_rate(A, B, C), 1 - gm.false_pos_rate(A, B, C)).all()
    accuracy=gm.Utility([1,0,0,1], 'accuracy')
    assert np.isclose(gm.accuracy(A, B, C), accuracy(A, B, C)).all()
    # assert np.isclose(gm.(A,B,C),1-gm.(A,B,C)).all()
    # check that additive_metrics can be called.
    assert np.isclose(gm.equalized_odds(A, B, C),
                        (gm.true_pos_rate.diff(A, B, C) + gm.true_neg_rate.diff(A, B, C)) / 2).all()
...
def no_groups(predictor, train_data):
    ...
    f = fair.FairPredictor(predictor, train_data)
    f.evaluate()
    f.evaluate_groups()
    f.evaluate_fairness()
    assert (f.predict_proba(train_data) == predictor.predict_proba(train_data)).all().all()
    assert (f.predict(train_data) == predictor.predict(train_data)).all().all()
    f.fit(gm.accuracy, gm.f1, 0)
