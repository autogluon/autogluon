...
class ForecastingPredictor:
    ...
    **kwargs :
    ...
    def __init__(self, eval_metric=None, path=None, verbosity=2, **kwargs):
    ...
    def fit(
        ...
        **kwargs:
        ...
        if index_column is None:
            logger.log(
                30,
                "index_column=None, assuming there is only one time series in the dataset.",
            )
        ...
        if isinstance(train_data, pd.DataFrame):
            ...
            extra_columns = [
                c
                for c in train_data.columns.copy()
                if c not in [index_column, time_column, target_column]
            ]
            if len(extra_columns) > 0:
                logger.log(
                    30,
                    f"Find more than 3 columns, columns {extra_columns} will not be used.",
                )
            train_data = rebuild_tabular(
                train_data,
            ...
            train_data = TimeSeriesDataset(
                train_data, index_column=index_column, static_features=static_features
            )
            ...
            self.prev_inferred_static_features = {
                "static_cat_columns": self.static_cat_columns,
            ...
            if val_data is None:
                logger.log(
                    30, "Validation data is not specified, will do auto splitting..."
                )
                train_data, val_data = train_test_split_gluonts(
                    train_data, prediction_length, freq
                )
        else:
    ...
    def preprocessing(self, data, time_series_to_predict=None, static_features=None):
        ...
        """
        if (
            self.use_feat_static_cat or self.use_feat_static_real
        ) and static_features is None:
            raise ValueError(
                "Static features are used for training, cannot predict without static features."
            )
    ...
    def predict(
        ...
        **kwargs,
        ...
        processed_data = self.preprocessing(
            data,
            time_series_to_predict=time_series_to_predict,
            static_features=static_features,
        )
        predict_targets = self._learner.predict(
            processed_data, model=model, for_score=for_score, **kwargs
        )
    ...
    def _post_fit(
        self, keep_only_best=False, refit_full=False, set_best_to_refit_full=False
    ):
        """
        Post fit operations.
        """
        ...
        if refit_full is not False:
            ...
            if set_best_to_refit_full:
                if trainer_model_best in self._trainer.model_full_dict.keys():
                    ...
                    logger.warning(
                        f"Best model ({trainer_model_best}) is not present in refit_full dictionary. "
