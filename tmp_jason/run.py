...
from autogluon.tabular.models import CatBoostModel, KNNModel
import pandas as pd
...
train_data = pd.read_csv(args.train_path)  # .head(1000)
...
if args.prune:
    predictor = predictor.fit(train_data, presets=presets, ag_args=fit_with_prune_kwargs,
                              ag_args_ensemble={'use_child_oof': False}, hyperparameters=custom_hyperparameters)
else:
    predictor = predictor.fit(train_data, presets=presets, hyperparameters=custom_hyperparameters)
