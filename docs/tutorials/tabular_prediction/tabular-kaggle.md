# How to easily enter Kaggle contests with AutoGluon  
...
**Tips to maximize predictive performance:**
   - Be sure here to specify the appropriate evaluation metric if one is specified on the competition website! If you are unsure which metric is best, then simply do not specify this argument when invoking `fit()`; AutoGluon should still produce high-quality models by automatically inferring which metric to use.
   ...
   - Specify the following `fit()` arguments: `num_bagging_folds`, `stack_ensemble_levels` (we recommend trying values 5-10 for the former, 1-2 for the latter). Note these choices will however increase the runtime of `fit()`.
   
**Troubleshooting tips:**

- Check that you have the right permissions to access the downloaded data files on your computer.

- If you have issues downloading Kaggle data or submitting predictions, check that your Kaggle account is properly set up and browse [Kaggle FAQ](https://www.kaggle.com/general/14438).
