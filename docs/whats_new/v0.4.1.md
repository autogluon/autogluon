...
We're happy to announce the AutoGluon 0.4.1 release. 0.4.1 contains minor enhancements to Tabular and Text modules, along with many quality of life improvements and fixes.
...
This release contains [**29** commits from **10** contributors](https://github.com/awslabs/autogluon/graphs/contributors?from=2022-03-10&to=2022-05-23&type=c)!
...
- Updated the `standalone` flag in `automm.predictor.save()` to to save the pertained model for offline deployment (#1575) @yiqings
...
- Updated feature importance default `subsample_size` 1000 -> 5000, `num_shuffle_sets 3` -> 5 (#1708) @Innixma
  - This will improve the quality of the feature importance values by default, especially the 99% confidence bounds. The change increases the time taken by ~8x, but this is acceptable because of the numerous inference speed optimizations are done since these defaults were first introduced.
...
- Fixed repeated bagging failing to fit repeats beyond the first when time limit is specified due to `self.model_base` being `None` in the loaded model (#1702) @Innixma
...
- Disabled image/text predictor when gpu is not avaialble (#1676) @yinweisu
