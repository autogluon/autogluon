...
### AutoMM Highlights in One Figure
![AutoMM highlights](https://automl-mm-bench.s3.amazonaws.com/figures/automm-intro.png)

### AutoMM Uniqueness
AutoMM distinguishes itself from other open-source AutoML toolboxes like FLAMAL, autosklearn, lightautoml, and TPOT, which mainly focus on tabular data using methods such as boosted trees and KNN for classification or regression. 
AutoMM is uniquely designed for fine-tuning foundation models across multiple modalities—image, text, tabular, and document, either individually or combined. 
...
In contrast, toolboxes like autosklearn, lightautoml, and TPOT have limited support for image or text, typically using a few pretrained models like EfficientNet or hand-crafted rules like bag-of-words as feature extractors. 
They often rely on traditional models or simple neural networks. 
AutoKeras, despite its emphasis on neural networks, confines its support to image or a single combination of image + tabular data exclusively for classification or regression tasks. 
