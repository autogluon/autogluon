...
Special thanks to @LennartPurucker for leading development of dynamic stacking, @geoalgo for helping with the Zeroshot-HPO feature, @ddelange for helping to add Python 3.11 support, and @mlgowacki100 for providing numerous comments and suggestions
...
Not only is AutoGluon accurate, it is fast too.
AutoGluon is capable of achieving the fastest inference throughput of any AutoML system while still obtaining state-of-the-art results.
By simply specifying the `infer_limit` fit argument, users can easily trade off between accuracy and inference speed to meet their needs.
AutoGluon 1.0 High can achieve similar performance to AutoGluon 0.8.2 Best with 8x faster inference and 8x less disk usage!
