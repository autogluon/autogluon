...
TODO: Discord / YouTube / Twitter

Today is finally the day... AutoGluon 1.0 has arrived!! After [over four years of development](https://automlpodcast.com/episode/autogluon-the-story) and [2052 commits from 111 contributors](https://github.com/autogluon/autogluon/graphs/contributors),
...
This release contains XXX commits from 17 contributors!

Special thanks to @LennartPurucker for leading development of dynamic stacking, @geoalgo for co-authoring [TabRepo](https://github.com/autogluon/tabrepo) to enable Zeroshot-HPO, @ddelange for helping to add Python 3.11 support, and @mlgowacki100 for providing numerous comments and suggestions
...
Dynamic stacking to [mitigate stacked overfitting](https://github.com/autogluon/autogluon/issues/2779),
and a new learned model hyperparameters portfolio via Zeroshot-HPO, obtained from the newly released [TabRepo](https://github.com/autogluon/tabrepo) ensemble simulation library.
Together, they lead to a **75% win-rate compared to 0.8.2 with faster inference speed, lower disk usage, and higher stability.**
...
Their results show AutoGluon 0.8.2 as the state-of-the-art in AutoML systems across a wide variety of tasks:
"Overall, in terms of model performance, AutoGluon consistently has the highest average rank in our benchmark."
We now showcase that AutoGluon 1.0 achieves far superior results even to AutoGluon 0.8.2!
...
**AutoGluon 1.0 achieves first place in 63% of tasks**, with lightautoml having the second most at 12% (AutoGluon 0.8.2 previously took first place 48% of the time).
AutoGluon 1.0 even achieves a 7.4% average loss improvement over AutoGluon 0.8.2!
...
AutoGluon 1.0 High achieves superior performance to AutoGluon 0.8.2 Best with 8x faster inference and 8x less disk usage!

![AutoGluon 1.0 AutoML Benchmark Plot](https://raw.githubusercontent.com/Innixma/autogluon-benchmark/master/v1_results/pareto_front_all.png)
...
As always, we will continue to refine and enhance AutoGluon in future releases to push the boundaries of AutoML forward for all.

## Spotlight: Newly Published Papers

### AutoGluon-TimeSeries: AutoML for Probabilistic Time Series Forecasting

We have published a paper on AutoGluon-TimeSeries at [AutoML Conference 2023](https://openreview.net/forum?id=XHIY3cQ8Tew) ([YouTube Video](https://www.youtube.com/watch?v=niLmfjXeHnE)).
...
AutoGluon produces SOTA results in point & probabilistic forecasting, and even **achieves 65% win rate against the best-in-hindsight combination of models**.
...
We have published a paper on tabular Transformer pre-training at ICML 2023 ([Paper Link](https://arxiv.org/abs/2305.06090)).
...
While the pre-trained transformer is not yet incorporated into AutoGluon, we plan to integrate it into AutoGluon in a future release.
...
- We have published a paper on AutoGluon-TimeSeries at [AutoML Conference 2023](https://openreview.net/forum?id=XHIY3cQ8Tew). In the paper, we benchmarked AutoGluon and popular open-source forecasting frameworks (including DeepAR, TFT, AutoARIMA, AutoETS, AutoPyTorch). AutoGluon produces SOTA results in point & probabilistic forecasting, and even achieves 65% win rate against the best-in-hindsight combination of models.
