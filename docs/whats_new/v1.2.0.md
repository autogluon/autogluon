...
### Highlights
AutoGluon now features Chronos⚡ (Chronos-Bolt) — new, more accurate, and up to 250x faster Chronos models that can also run on CPU. The new release also adds fine-tuning and covariate handling support for all Chronos models ([tutorial](https://auto.gluon.ai/stable/tutorials/timeseries/forecasting-chronos.html)). These additions as well as new presets result in significantly improved forecast accuracy — AutoGluon v1.2 achieves a 70%+ win rate against the previous release.
