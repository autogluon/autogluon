...
job_type_info = {
    "CI-CPU": {"job_definition": "autogluon-ci-cpu:3", "job_queue": "CI-CPU"},
    "CI-GPU": {"job_definition": "autogluon-ci-gpu:3", "job_queue": "CI-GPU"},
    "CI-WASM": {"job_definition": "autogluon-ci-wasm:1", "job_queue": "CI-CPU"},
    "CI-MULTI-GPU": {"job_definition": "autogluon-ci-multi-gpu:6", "job_queue": "CI-MULTI-GPU"},
    "CI-CPU-PUSH": {"job_definition": "autogluon-ci-cpu-push:3", "job_queue": "CI-CPU"},
    "CI-GPU-PUSH": {"job_definition": "autogluon-ci-gpu-push:4", "job_queue": "CI-GPU"},
    "CI-WASM-PUSH": {"job_definition": "autogluon-ci-wasm-push:2", "job_queue": "CI-CPU"},
    "CI-MULTI-GPU-PUSH": {"job_definition": "autogluon-ci-multi-gpu-push:2", "job_queue": "CI-MULTI-GPU"},
}
...
parser.add_argument(
    "--job-type", help="type of job to submit.", type=str, choices=job_type_info.keys(), default="CI-CPU"
)
parser.add_argument(
    "--source-ref", help="ref in AutoGluon main github. e.g. master, refs/pull/500/head", type=str, default="master"
)
parser.add_argument(
    "--work-dir",
...
parser.add_argument(
    "--saved-output",
    help="output to be saved, relative to working directory. it can be either a single file or a directory",
    type=str,
    default="None",
)
parser.add_argument(
    ...
    type=str,
    default="batch/temp/{}".format(datetime.now().isoformat()),
)
...
parser.add_argument(
    "--wait", help="block wait until the job completes. Non-zero exit code if job fails.", action="store_true"
)
...
def main():
    ...
    while wait:
        ...
        if status == "SUCCEEDED" or status == "FAILED":
            ...
            print(
                "\rJob [%s - %s] is %-9s... %s" % (jobName, jobId, status, spin[spinner % len(spin)]),
            )
            sys.stdout.flush()
            spinner += 1
