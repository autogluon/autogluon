...
def get_parser():
    ...
    parser.add_argument("--train_file", type=str, help="The training pandas dataframe.", default=None)
    parser.add_argument("--dev_file", type=str, help="The validation pandas dataframe", default=None)
    parser.add_argument("--test_file", type=str, help="The test pandas dataframe", default=None)
    ...
    parser.add_argument(
        "--all_metrics",
        type=str,
        help="All metrics that we will report. This will usually include the eval_metric",
        default=None,
    )
...
def train(args):
    ...
    if args.task == "mrpc" or args.task == "sts":
        ...
        train_df_other_part = pd.DataFrame(
            {
                feature_columns[0]: train_df[feature_columns[1]],
    ...
    if args.mode == "stacking":
        predictor = TabularPredictor(label=label_column, eval_metric=eval_metric, path=args.exp_dir)
        predictor.fit(
            train_data=real_train_df,
        ...
        predictor = TabularPredictor(label=label_column, eval_metric=eval_metric, path=args.exp_dir)
        predictor.fit(train_data=real_train_df, tuning_data=real_dev_df, hyperparameters=hyperparameters)
    elif args.mode == "single":
        # When no embedding is used,
        # we will just use MultiModalPredictor that will train a single model internally.
        predictor = MultiModalPredictor(label=label_column, eval_metric=eval_metric, path=args.exp_dir)
        predictor.fit(train_data=real_train_df, tuning_data=real_dev_df, presets=args.preset, seed=args.seed)
    else:
        raise NotImplementedError
