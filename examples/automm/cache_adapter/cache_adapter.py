"""
    The derivative application of Tip-Adapter (https://arxiv.org/pdf/2111.03930.pdf).
...
def get_args():
    parser = argparse.ArgumentParser(description="MultiModal Fewshot training with cache adapter.")
    parser.add_argument(
        "--type",
        type=str,
        default="clip", 
    ...
    parser.add_argument("--lr_F", type=float, default=1e-3, help="The learning rate for finetuing the cache adapter.")
    parser.add_argument("--train_epoch", type=int, default=20, help="The training epochs for training the model head.")
    parser.add_argument("--train_epoch_F", type=int, default=20, help="The training epochs for finetuning the cache adapter.")
    parser.add_argument("--init_alpha", type=float, default=1.17, help="The initial value of hyper-parameter alpha in cache adapter")
    parser.add_argument("--init_beta", type=float, default=1., help="The initial values of hyper-parameter beta in cache adapter.")
...
class AutoMMCacheAdapter(nn.Module):
    """
    The model to generate few shot predict proba with 
    ...
    def __init__(
        ...
        """
        Create the model head and the cache adapter.
    ...
    def get_adapter_logits(self, affinity, pure_logits, alpha, beta):
        """
        Generate logits with cache adapter based on pure_logits and cache output.
        ...
        pure_logits
            The predict proba of the classifier.
        alpha
    ...
    def change_adapter_state(self, grad_state):
        """
        Change the training state of cache adapter.

        Parameters
        ----------
        grad_state
            The needed training state of the cache adapter.
        """
    ...
    def forward(self, x, alpha = 1, beta = 1):
        ...
        Return
        ------
        The predict proba of the feature.
            - "pure_logits"
                The predict proba of classifier.
            - "logits_with_adapter"
                The predict proba composed of classifier and cache adapter similarity result.
            - "logits_with_finetuned_adpter"
                The predict proba composed of classifier and fine-tuned cache adapter result.
        """
...
def train_logits(
    ...
    """
    The training process of AutoMMCacheAdapter.
    ...
    adapter_model
        The AutoMMCacheAdapter to generate logits and logits with cache adapter.
    alpha, beta
        The hyper-parameters of cache adapter.
    loader
    ...
    if logits_type != "logits_with_finetuned_adapter":
        adapter_model.change_head_state(grad_state=True)
        adapter_model.change_adapter_state(grad_state=False)
        lr, train_epoch = args.lr, args.train_epoch
    else:
        adapter_model.change_head_state(grad_state=False)
        adapter_model.change_adapter_state(grad_state=True)
        lr, train_epoch = args.lr_F, args.train_epoch_F
            
    optimizer = torch.optim.AdamW(adapter_model.parameters(), lr=lr, eps=1e-4)
    ...
    for train_idx in range(train_epoch):
        ...
        for i, (data, target) in enumerate(tqdm(loader)):
            ...
            logits = adapter_model(features, alpha, beta)
        ...
        adapter_model.eval()
        logits = adapter_model(val_features, alpha, beta)[logits_type]
        acc = cls_acc(logits, val_labels)
        print("**** val accuracy: {:.2f}. ****\n".format(acc))
        if acc > best_acc:
            best_acc = acc
            best_epoch = train_idx
            torch.save(adapter_model.state_dict(), args.cache_dir + "/best_F_" + str(args.shots) + "shots.pt")
    
    adapter_model.load_state_dict(torch.load(args.cache_dir + "/best_F_" + str(args.shots) + "shots.pt"))
    print(f"**** After fine-tuning {logits_type}, best test accuracy: {best_acc:.2f}, at epoch: {best_epoch}. ****\n")
    
    return adapter_model


def run_cache_adapter(
    args, 
    val_features, 
    val_labels, 
    test_features, 
    ...
    """
    Test the effectiveness of cache adapter. 
    ...
    with torch.no_grad():
        logits = adapter_model(test_features, best_alpha, best_beta)
...
def main():
    ...
    adapter_model = AutoMMCacheAdapter(
