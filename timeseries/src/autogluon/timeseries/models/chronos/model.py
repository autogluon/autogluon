...
class ChronosModel(AbstractTimeSeriesModel):
    ...
    disable_static_features : bool, default = True
        If True, static features will not be used during fine-tuning even if they are present in the dataset
    disable_known_covariates : bool, default = True
        If True, known covariates will not be used during fine-tuning even if they are present in the dataset
    disable_past_covariates : bool, default = True
        If True, past covariates will not be used during fine-tuning even if they are present in the dataset
    """
    ...
    def __init__(
        ...
        **kwargs,  # noqa
        ...
        # Following attributes may be overridden during fit() based on train_data & model
        self.dynamic_dims = 0
        self.past_dynamic_dims = 0
        self.static_dims = 0
        self.static_cardinalities: List[int] = []
        self.dynamic_cardinalities: List[int] = []
        self.past_dynamic_cardinalities: List[int] = []
    ...
    def _deferred_init_covariate_hyperparameters(self, model_params: dict, dataset: TimeSeriesDataFrame) -> None:
        ...
        if not disable_static_features:
            ...
            if len(self.covariate_metadata.static_features_cat) > 0:
                ...
                self.static_cardinalities = feat_static_cat.nunique().tolist()

        disable_known_covariates = model_params.get("disable_known_covariates", False) or no_fine_tune
        if not disable_known_covariates and self.supports_known_covariates:
            self.dynamic_dims = len(self.covariate_metadata.known_covariates_real)
            if len(self.covariate_metadata.known_covariates_cat) > 0:
                feat_dynamic_cat = dataset[self.covariate_metadata.known_covariates_cat]
                # Assumes model support cat feats
                self.dynamic_cardinalities = feat_dynamic_cat.nunique().tolist()

        disable_past_covariates = model_params.get("disable_past_covariates", False) or no_fine_tune
        if not disable_past_covariates and self.supports_past_covariates:
            self.past_dynamic_dims = len(self.covariate_metadata.past_covariates_real)
            if len(self.covariate_metadata.past_covariates_cat) > 0:
                past_feat_dynamic_cat = dataset[self.covariate_metadata.past_covariates_cat]
                self.past_dynamic_cardinalities = past_feat_dynamic_cat.nunique().tolist()
    ...
    def _fit(
        ...
        **kwargs,
        ...
        self._deferred_init_covariate_hyperparameters(model_params, train_data)
