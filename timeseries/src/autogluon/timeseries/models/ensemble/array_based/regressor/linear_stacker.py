...
class LinearStackerEnsembleRegressor(EnsembleRegressor):
    """Linear stacker ensemble regressor using PyTorch optimization with
    softmax weights.

    Implements weighted averaging of base model predictions with learnable
    weights optimized via gradient descent. Uses PyTorch during training
    for optimization, then stores weights as numpy arrays for efficient
    prediction.

    Parameters
    ----------
    quantile_levels
        List of quantile levels for quantile predictions (e.g.,
        [0.1, 0.5, 0.9]).
    weights_per
        Weight configuration specifying which dimensions to learn weights
        for:
        - "m": Per-model weights (shape: num_models), defaults to "m"
        - "mt": Per-model and per-time weights
          (shape: prediction_length, num_models)
        - "mq": Per-model and per-model-output (quantiles and mean)
          weights (shape: num_quantiles+1, num_models)
    ...
    tolerance_change
        Convergence tolerance for loss change between epochs. Defaults to
        1e-7.
    tolerance_grad
        Convergence tolerance for maximum gradient magnitude. Defaults to
        1e-6.
    """
