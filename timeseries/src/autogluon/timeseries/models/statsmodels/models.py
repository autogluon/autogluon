...
class SeasonalNaiveModel(AbstractStatsmodelsModel):
    """Predicted value is set to the last observed value from the same season (e.g., the same month of the previous year).

    Does not produce a quantile forecast (i.e., all quantiles are set equal to the mean).
    ...
    seasonal_period : int or None, default = None
        ...
        When set to None, seasonal_period will be inferred from the frequency of the training data. Can also be
        specified manually by providing an integer > 1.
        If length of the training series is less than seasonal_period, will predict the mean value.
    """
    ...
    def _update_sm_model_init_args(self, sm_model_init_args: dict, data: TimeSeriesDataFrame) -> dict:
        sm_model_init_args = sm_model_init_args.copy()
        seasonal_period = sm_model_init_args.get("seasonal_period", None)
        if seasonal_period is None:
            sm_model_init_args["seasonal_period"] = get_seasonality(data.freq)
        return sm_model_init_args
    ...
    def _predict_with_local_model(
        ...
        seasonal_period = fitted_model.sm_model_init_args["seasonal_period"]
        start_ts = timeseries.index.max() + pd.tseries.frequencies.to_offset(self.freq)
        forecast_index = pd.date_range(start=start_ts, periods=self.prediction_length, freq=self.freq)

        target = timeseries.values
        forecast = {}
        if len(target) >= seasonal_period:
            indices = [len(target) - seasonal_period + k % seasonal_period for k in range(self.prediction_length)]
            forecast["mean"] = target[indices].reshape(-1)
        else:
            forecast["mean"] = np.full(shape=[self.prediction_length], fill_value=target.mean())
