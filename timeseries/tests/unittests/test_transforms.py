...
@pytest.mark.parametrize("scaler_type", SCALER_TYPES)
def test_when_model_fit_then_fit_transform_called_once(model_class, scaler_type):
    data = DUMMY_TS_DATAFRAME.copy()
    model = model_class(
        prediction_length=4,
        freq=data.freq,
        hyperparameters={"max_epochs": 1, "num_batches_per_epoch": 1, "target_transform": scaler_type},
    )
    with mock.patch(
        "autogluon.timeseries.transforms.scaler.LocalTargetScaler.fit_transform", side_effect=lambda x: x
