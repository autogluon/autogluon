...
def test_given_revision_when_model_saved_and_loaded_then_revision_is_preserved(temp_model_path, chronos_model_path):
    ...
    loaded_model = ChronosModel.load(path=model.path)

    assert loaded_model.get_hyperparameters()["revision"] == revision_id
    # Ensure attributes are preserved after loading
    assert loaded_model.device == "cpu"
    assert loaded_model.torch_dtype == "auto"
...
def test_given_legacy_model_without_revision_when_loaded_then_defaults_to_none(chronos_model_path):
    # Simulate a legacy model that doesn't have the 'revision' attribute
    model = ChronosModel(hyperparameters={"model_path": chronos_model_path, "device": "cpu"})
    # Speed optimization:
    # We manually set attributes to simulate a configured model state.
    # Calling fit() is unnecessary overhead just to verify serialization logic.
    model.device = "cpu"
    model.torch_dtype = "auto"
    # Forcefully delete the attribute to mimic an old pickled object
    ...
    with mock.patch("chronos.BaseChronosPipeline.from_pretrained") as mock_from_pretrained:
        ...
        # This would raise AttributeError if `getattr` wasn't used
        model.load_model_pipeline()

    mock_from_pretrained.assert_called_once()
    # Assert that the default behavior (revision=None) is triggered
