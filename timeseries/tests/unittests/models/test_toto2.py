...
class MockToto2Model:
    ...
    def forecast(self, inputs, horizon, decode_block_size=None, has_missing_values=True):
...
class TestToto2Dataloader:
    ...
    @pytest.mark.parametrize("device", ["cpu", "cuda:0"])
    def test_when_pad_to_multiple_set_then_context_length_is_divisible(self, pad_to_multiple, device):
        if device == "cuda:0" and not torch.cuda.is_available():
            pytest.skip(reason="No GPU available")

        # 50 is not a multiple of 16 or 32
        df = get_data_frame_with_item_index(["A", "B", "C", "D"], data_length=50)
