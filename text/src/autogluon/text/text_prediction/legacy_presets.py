...
def list_presets():
    ...
    simple_presets = ["default", "lower_quality_fast_train", "medium_quality_faster_train", "best_quality"]
    return {
        "simple_presets": simple_presets,
        "advanced_presets": [key for key in ag_text_presets.list_keys() if key not in simple_presets],
    }
...
for backbone_name in ["electra_small", "electra_base", "electra_large", "roberta_base", "multi_cased_bert_base"]:
    for fusion_strategy in ["fuse_late"]:
        ag_text_presets.register(
            f"{backbone_name}_{fusion_strategy}",
            functools.partial(
                gen_config_no_hpo, backbone_name=backbone_name, nbest=3, fusion_strategy=fusion_strategy
            ),
        )
