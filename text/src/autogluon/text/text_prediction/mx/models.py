...
from autogluon_contrib_nlp.utils.misc import grouper, count_parameters, repeat, get_mxnet_available_ctx
...
from .preprocessing import (
    ...
    base_preprocess_cfg,
    MultiModalTextBatchify,
    get_stats_string,
    auto_shrink_max_length,
    get_cls_sep_id,
)
...
def get_optimizer(cfg, updates_per_epoch):
    ...
    optimizer_params = {"learning_rate": cfg.lr, "wd": cfg.wd, "lr_scheduler": lr_scheduler}
...
def base_optimization_config():
    ...
    cfg.optimizer_params = [("beta1", 0.9), ("beta2", 0.999), ("epsilon", 1e-6), ("correct_bias", False)]
    cfg.begin_lr = 0.0
    cfg.batch_size = 128
...
def _classification_regression_predict(
    net, dataloader, problem_type, label_scaler, has_label=True, extract_embedding=False, num_repeat=1
):
    """
    ...
    use_logits = num_repeat > 1 and (problem_type == MULTICLASS or problem_type == BINARY) and not extract_embedding
    if use_logits:
...
def train_function(
    ...
    if "roberta" in cfg.model.backbone.name:
        backbone_model_cls, backbone_cfg, tokenizer, backbone_params_path, _ = get_backbone(cfg.model.backbone.name)
        ...
        backbone_model_cls, backbone_cfg, tokenizer, backbone_params_path, _ = get_backbone(
            cfg.model.backbone.name, load_backbone=not continue_training
        )
    ...
    if continue_training:
        ...
        train_dataset = preprocessor.transform(train_data[feature_columns], train_data[label_column])
        ...
        preprocessor = MultiModalTextFeatureProcessor(
        ...
        train_dataset = preprocessor.fit_transform(train_data[feature_columns], train_data[label_column])
    ...
    tuning_dataset = preprocessor.transform(tuning_data[feature_columns], tuning_data[label_column])
    logger.info("Done!")
    # Auto Max Length
    ...
    with open(os.path.join(exp_dir, "cfg.yml"), "w") as f:
        f.write(cfg.dump())
    logger.info(
        f"Max length for chunking text: {max_length}, "
    ...
    train_batchify_fn = MultiModalTextBatchify(
        ...
        mode="train",
        stochastic_chunk=train_stochastic_chunk,
        insert_sep=cfg.model.insert_sep,
    )
    test_batchify_fn = MultiModalTextBatchify(
        ...
        max_length=max_length,
        mode="test",
        stochastic_chunk=test_stochastic_chunk,
        insert_sep=cfg.model.insert_sep,
    )
    ...
    if problem_type == REGRESSION:
        gt_dev_labels = preprocessor.label_scaler.inverse_transform(np.expand_dims(gt_dev_labels, axis=-1))[:, 0]
    ...
    train_dataloader = DataLoader(
        train_dataset, batch_size=base_batch_size, shuffle=True, batchify_fn=train_batchify_fn
    )
    dev_dataloader = DataLoader(
        tuning_dataset, batch_size=inference_base_batch_size, shuffle=False, batchify_fn=test_batchify_fn
    )
    if problem_type == REGRESSION:
        out_shape = 1
    ...
    net = MultiModalWithPretrainedTextNN(
        ...
        numerical_input_units=None
        if len(preprocessor.numerical_feature_names) == 0
    ...
    logger.info("#Total Params/Fixed Params={}/{}".format(num_total_params, num_total_fixed_params))
    # Initialize the optimizer
    updates_per_epoch = int(np.ceil(len(train_dataloader) / (num_accumulated * len(ctx_l))))
    optimizer, optimizer_params, max_update = get_optimizer(cfg.optimization, updates_per_epoch=updates_per_epoch)
    ...
    if 0 < cfg.optimization.layerwise_lr_decay < 1:
        apply_layerwise_decay(
            net.text_backbone, cfg.optimization.layerwise_lr_decay, backbone_name=cfg.model.backbone.name
        )
    freeze_layers(
        net.text_backbone, backbone_name=cfg.model.backbone.name, num_trainable_layers=cfg.model.num_trainable_layers
    )
    ...
    trainer = mx.gluon.Trainer(params, optimizer, optimizer_params, update_on_kvstore=False)
    # Set grad_req if gradient accumulation is required
    if num_accumulated > 1:
        logger.log(15, "Using gradient accumulation." " Global batch size = {}".format(cfg.optimization.batch_size))
        for p in params:
            p.grad_req = "add"
    ...
    logger.info(f"Local training results will be saved to " f'{os.path.join(exp_dir, "results_local.jsonl")}.')
    for update_idx in range(max_update):
            ...
            for i, (sample, ctx) in enumerate(zip(sample_l, ctx_l)):
                ...
                with mx.autograd.record():
                    ...
                    if problem_type == MULTICLASS or problem_type == BINARY:
                        ...
                        loss = -mx.npx.pick(logits, mx.np.expand_dims(label_batch, axis=-1))
        ...
        if (update_idx + 1) % train_log_interval == 0:
            ...
            logger.log(
                ...
                " #sample per second={:.2f}. ETA={:.2f}min".format(
                    update_idx + 1,
                    max_update,
                    ...
                    (time.time() - start_tick) / (update_idx + 1) * (max_update - update_idx - 1) / 60,
                ),
            )
            logging_start_tick = time.time()
            log_loss_l = [mx.np.array(0.0, dtype=np.float32, ctx=ctx) for ctx in ctx_l]
        ...
        if (update_idx + 1) % valid_interval == 0 or (update_idx + 1) == max_update:
            ...
            log_scores = [
                calculate_metric(scorer, gt_dev_labels, dev_predictions, problem_type) for scorer in log_metric_scorers
            ]
            dev_score = calculate_metric(eval_metric_scorer, gt_dev_labels, dev_predictions, problem_type)
            ...
            if len(best_performance_score) < nbest:
                ...
                net.save_parameters(os.path.join(exp_dir, f"nbest_model{len(best_performance_score) - 1}.params"))
                find_topn_better = True
                if (
                    best_score is None
                    or greater_is_better
                    and dev_score >= best_score
                ...
                if greater_is_better:
                    if dev_score >= min(best_performance_score):
                        ...
                        net.save_parameters(os.path.join(exp_dir, f"nbest_model{replace_idx}.params"))
            ...
            mx.npx.waitall()
            loss_string = ", ".join(
                ["{}={:0.4e}".format(metric.name, score) for score, metric in zip(log_scores, log_metric_scorers)]
            )
            logger.log(
                25,
                "[Iter {}/{}, Epoch {}] Validation {}, Time computing validation-score={:.3f}s,"
            ...
            if reporter is not None:
                report_items = (
                    ...
                    + [(metric.name, score) for score, metric in zip(log_scores, log_metric_scorers)]
                    + [
                        ("find_better", find_better),
                        ("find_new_topn", find_topn_better),
                        ("nbest_stat", json.dumps([best_performance_score, best_performance_update_idx])),
...
def get_recommended_resource(nthreads_per_trial=None, ngpus_per_trial=None) -> Tuple[int, int]:
    ...
    assert nthreads_per_trial > 0 and ngpus_per_trial >= 0, "Invalid number of threads and number of GPUs."
...
class MultiModalTextModel:
    ...
    def train(
        ...
        if search_space is None:
            search_space = ag_text_presets.create("default")["models"]["MultimodalTextModel"]["search_space"]
        ...
        logger.info(
            f"The GluonNLP V0 backend is used. "
        ...
        if scheduler_options["num_trials"] == 1:
            ...
            if not os.path.exists(cfg_path) or not os.path.exists(
                os.path.join(best_model_saved_dir_path, "best_model.params")
            ):
                raise RuntimeError(no_job_finished_err_msg)
            cfg = self.base_config.clone_merge(cfg_path)
            local_results = pd.read_json(
                os.path.join(self._output_directory, "task0", "results_local.jsonl"), lines=True
            )
            if plot_results:
                ...
                plt.plot(
                    local_results["report_idx"].iloc[:-1],
                    local_results[local_results["eval_metric"][0]].iloc[:-1],
                    label=f"task0",
                )
                plt.legend(loc="best")
            ...
            scheduler = scheduler_cls(
                train_function, search_space=search_space, train_fn_kwargs=train_fn_kwargs, **scheduler_params
            )
            scheduler.run()
            scheduler.join_jobs()
            ...
            best_model_saved_dir_path = os.path.join(self._output_directory, "task{}".format(best_task_id))
            best_cfg_path = os.path.join(best_model_saved_dir_path, "cfg.yml")
            cfg = self.base_config.clone_merge(best_cfg_path)
            if plot_results:
                plot_training_curves = os.path.join(self._output_directory, "plot_training_curves.png")
                scheduler.get_training_curves(filename=plot_training_curves, plot=plot_results, use_legend=True)
            self._results = dict()
        ...
        net = MultiModalWithPretrainedTextNN(
            ...
            numerical_input_units=None
            if len(self._preprocessor.numerical_feature_names) == 0
            else len(self._preprocessor.numerical_feature_names),
        ...
        if cfg.model.use_avg_nbest:
            ...
            net.load_parameters(os.path.join(best_model_saved_dir_path, "best_model.params"), ctx=mx.cpu())
        self._net = net
        mx.npx.waitall()
    ...
    def evaluate(self, data, metrics=None, stochastic_chunk=None, num_repeat=None):
        ...
        if not isinstance(data, pd.DataFrame):
            if isinstance(data, (list, dict)):
                ...
                raise NotImplementedError(
                    f"The format of data is not understood. " f'We have type(data)="{type(data)}"'
                )
        ...
        if self._problem_type == MULTICLASS or self._problem_type == BINARY:
            ...
            predictions = self.predict_proba(data, stochastic_chunk=stochastic_chunk, num_repeat=num_repeat)
        else:
            ground_truth = pd.to_numeric(data[self._label_columns[0]]).to_numpy().astype(np.float32)
            predictions = self.predict(data, stochastic_chunk=stochastic_chunk, num_repeat=num_repeat)
        metric_scores = [
            calculate_metric(get_metric(metric), ground_truth, predictions, self._problem_type) for metric in metrics
        ]
    ...
    def _internal_predict(
        self, data, get_original_labels=True, get_probabilities=False, stochastic_chunk=None, num_repeat=None
    ):
        assert self.net is not None
        ...
        inference_batch_size = (
            self.config.optimization.per_device_batch_size * self.config.optimization.val_batch_size_mult
        )
        ...
        dataloader = DataLoader(dataset, batch_size=inference_batch_size, shuffle=False, batchify_fn=batchify_fn)
        ...
        if self._problem_type == MULTICLASS or self._problem_type == BINARY:
            if get_probabilities:
                ...
                if get_original_labels:
                    test_predictions = np.array(self.preprocessor.label_generator.inverse_transform(test_predictions))
        return test_predictions
    ...
    def load(cls, dir_path: str):
        ...
        if "roberta" in cfg.model.backbone.name:
            ...
            backbone_model_cls, backbone_cfg, tokenizer, backbone_params_path, _ = get_backbone(
                cfg.model.backbone.name, load_backbone=False
            )
        if "roberta" in cfg.model.backbone.name:
            text_backbone = backbone_model_cls.from_cfg(backbone_cfg, return_all_hiddens=True)
        else:
    ...
    def extract_embedding(self, data, stochastic_chunk=None, num_repeat=None):
        ...
        if self._embed_net is None:
            embed_net = MultiModalWithPretrainedTextNN(
                ...
                numerical_input_units=None
                if len(self.preprocessor.numerical_feature_names) == 0
