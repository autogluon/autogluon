...
from autogluon.text.mx.preprocessing import (
...
from autogluon.text.infer_types import infer_column_problem_types
...
@pytest.mark.parametrize("backbone_name", ["google_electra_small", "google_albert_base_v2"])
@pytest.mark.parametrize("all_to_text", [False, True])
def test_preprocessor(dataset_name, url, label_column, backbone_name, all_to_text):
    all_df = load_pd.load(url)
    feature_columns = [col for col in all_df.columns if col != label_column]
    train_df, valid_df = train_test_split(all_df, test_size=0.1, random_state=np.random.RandomState(100))
    column_types, problem_type = infer_column_problem_types(train_df, valid_df, label_columns=label_column)
    ...
    preprocessor = MultiModalTextFeatureProcessor(
        column_types=column_types, label_column=label_column, tokenizer_name=backbone_name, cfg=cfg
    )
    ...
    for i in range(len(train_dataset)):
        for j in range(len(train_dataset[0])):
            npt.assert_allclose(train_dataset[i][j], train_dataset_after_transform[i][j], 1e-4, 1e-4)
    ...
    for i in range(len(test_dataset)):
        for j in range(len(test_dataset[0])):
            npt.assert_allclose(valid_dataset[i][j], test_dataset[i][j], 1e-4, 1e-4)
    ...
    with tempfile.TemporaryDirectory() as tmp_dir_name:
        ...
        valid_dataset_loaded = preprocessor_loaded.transform(valid_df[feature_columns], valid_df[label_column])
...
@use_np
@pytest.mark.parametrize("dataset_name,url,label_column", TEST_CASES)
@pytest.mark.parametrize("backbone_name", ["google_electra_small", "google_albert_base_v2", "fairseq_roberta_base"])
@pytest.mark.parametrize("all_to_text", [False, True])
@pytest.mark.parametrize("insert_sep", [False, True])
@pytest.mark.parametrize("stochastic_chunk", [False, True])
def test_multimodal_batchify(
    dataset_name, url, label_column, backbone_name, all_to_text, insert_sep, stochastic_chunk
):
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=4, batchify_fn=train_batchify_fn, shuffle=True)
    test_dataloader = DataLoader(test_dataset, batch_size=4, batchify_fn=test_batchify_fn, shuffle=False)
    for sample in train_dataloader:
        features, label = sample[0], sample[1]
        assert (
            len(features)
            == train_batchify_fn.num_text_outputs
            + train_batchify_fn.num_categorical_outputs
            + train_batchify_fn.num_numerical_outputs
        )
    ...
    for sample in test_dataloader:
        assert (
            len(sample)
            == test_batchify_fn.num_text_outputs
            + test_batchify_fn.num_categorical_outputs
            + test_batchify_fn.num_numerical_outputs
        )
