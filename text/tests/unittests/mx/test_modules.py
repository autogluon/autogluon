...
from autogluon.text.mx.modules import MultiModalWithPretrainedTextNN


@use_np
@pytest.mark.parametrize(
    "num_text_features,num_categorical_features,num_numerical_features", [(3, 4, 1), (1, 0, 0), (1, 4, 0), (2, 0, 1)]
)
@pytest.mark.parametrize("aggregate_categorical", [False, True])
@pytest.mark.parametrize("agg_type", ["mean", "concat", "max", "attention", "attention_token"])
@pytest.mark.parametrize("out_shape", [2])
def test_multimodal_with_pretrained_text_nn(
    num_text_features, num_categorical_features, num_numerical_features, aggregate_categorical, agg_type, out_shape
):
    ...
    backbone_model_cls, backbone_cfg, tokenizer, backbone_params_path, _ = get_backbone("google_electra_small")
    ...
    for i in range(num_text_features):
        text_features.append(
            (
                mx.np.random.randint(0, 100, (batch_size, seq_length)),
