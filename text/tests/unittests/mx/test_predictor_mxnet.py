...
from autogluon.text.legacy_presets import ag_text_presets
from autogluon.text.constants import MXNET
...
def verify_predictor_save_load(predictor, df, verify_proba=False, verify_embedding=True):
...
def test_predictor_fit(key):
    ...
    predictor.fit(train_data, hyperparameters=get_test_hyperparameters(), time_limit=30, seed=123)
...
def test_cpu_only_raise(set_env_train_without_gpu):
    ...
    if set_env_train_without_gpu is None:
        with pytest.raises(RuntimeError):
            predictor.fit(train_data, hyperparameters=get_test_hyperparameters(), num_gpus=0, seed=123)
    elif set_env_train_without_gpu is True:
        os.environ["AUTOGLUON_TEXT_TRAIN_WITHOUT_GPU"] = "1"
        predictor.fit(train_data, hyperparameters=get_test_hyperparameters(), num_gpus=0, time_limit=30, seed=123)
...
def test_no_text_column_raise():
    data = [("😁😁😁😁😁😁", "grin")] * 2000 + [("😃😃😃😃😃😃😃😃", "smile")] * 1000 + [("😉😉😉", "wink")] * 1000
...
def test_emoji():
    ...
    predictor.fit(df, hyperparameters=get_test_hyperparameters(), time_limit=30, seed=123)
...
def test_mixed_column_type():
    ...
    predictor1.fit(train_data, hyperparameters=get_test_hyperparameters(), time_limit=30, seed=123)
    ...
    predictor2.fit(train_data, hyperparameters=get_test_hyperparameters(), time_limit=30, seed=123)
