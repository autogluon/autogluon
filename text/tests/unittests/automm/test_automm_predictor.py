...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ae", ["hf_text"], "prajjwal1/bert-tiny", None, BEST, NORM_FIT),
...
def test_predictor(dataset_name, model_names, text_backbone, image_backbone, top_k_average_method, efficient_finetune):
    ...
    predictor = AutoMMPredictor(
        label=dataset.label_columns[0], problem_type=dataset.problem_type, eval_metric=metric_name,
    )
    config = {
    ...
    predictor.fit(
        train_data=dataset.train_df, config=config, hyperparameters=hyperparameters, time_limit=30,
    )
    verify_predictor_save_load(predictor, dataset.test_df)
...
def test_standalone():  # test standalong feature in AutoMMPredictor.save()
    ...
    predictor.save(
        path=save_path_standalone, standalone=True,
    )
...
def test_customizing_model_names(hyperparameters,):
    ...
    hyperparameters.update(
        {"env.num_workers": 0, "env.num_workers_evaluation": 0,}
    )
    ...
    # Test for continuous fit
    predictor.fit(
        train_data=dataset.train_df, config=config, hyperparameters=hyperparameters, time_limit=10,
    )
    assert sorted(predictor._config.model.names) == sorted(hyperparameters_gt["model.names"])
    for per_name in hyperparameters_gt["model.names"]:
        assert hasattr(predictor._config.model, per_name)
...
def test_modifying_duplicate_model_names():
    ...
    teacher_predictor.fit(
        train_data=dataset.train_df, config=config, time_limit=1,
    )
    student_predictor = AutoMMPredictor(
        label=dataset.label_columns[0], problem_type=dataset.problem_type, eval_metric=metric_name,
    )
    student_predictor.fit(
        train_data=dataset.train_df, config=config, time_limit=0,
    )

    teacher_predictor = modify_duplicate_model_names(
        predictor=teacher_predictor, postfix="teacher", blacklist=student_predictor._config.model.names,
    )
...
def test_textagumentor_deepcopy():
    ...
    hyperparameters = {
        ...
        "model.hf_text.text_train_augment_types": [
            "synonym_replacement({'aug_p': 0.05})",
