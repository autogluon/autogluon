...
import copy
import logging
from ...utils import default_holdout_frac, generate_train_test_split
...
class LinearAggregatorModel(StackerEnsembleModel):
    ...
    def _hyperparameter_tune(self, X, y, scheduler_options, compute_base_preds=True, **kwargs):
        ...
        X, X_val, y, y_val = generate_train_test_split(X, y, problem_type=self.problem_type,
                                                       test_size=default_holdout_frac(len(X)), random_state=self._random_state)
