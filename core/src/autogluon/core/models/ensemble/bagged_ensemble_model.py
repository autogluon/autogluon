...
class BaggedEnsembleModel(AbstractModel):
    ...
    def _fit(self, X, y_train, k_fold=5, k_fold_start=0, k_fold_end=None, n_repeats=1, n_repeat_start=0, time_limit=None, sample_weight=None, **kwargs):
        if k_fold < 1:
            k_fold = 1
        ...
        if k_fold == 1:
            ...
            model_base.fit(X=X, y_train=y_train, time_limit=time_limit, **kwargs)
        ...
        kfolds = generate_kfold(X=X, y=y_train, n_splits=k_fold, stratified=self.is_stratified(), random_state=self._random_state, n_repeats=n_repeats)

        oof_pred_proba, oof_pred_model_repeats = self._construct_empty_oof(X=X, y=y_train)
        ...
        for j in range(n_repeat_start, n_repeats):  # For each n_repeat
            ...
            for i in range(fold_start_n_repeat, fold_end_n_repeat):  # For each fold
                ...
                fold_model.fit(X=X_fold, y_train=y_train_fold, X_val=X_val_fold, y_val=y_val_fold, time_limit=time_limit_fold, **kwargs_fold)
    ...
    def _hyperparameter_tune(self, X, y_train, k_fold, scheduler_options, preprocess_kwargs=None, **kwargs):
        ...
        kfolds = generate_kfold(X=X, y=y_train, n_splits=k_fold, stratified=self.is_stratified(), random_state=self._random_state, n_repeats=1)
        ...
        hpo_models, hpo_model_performances, hpo_results = self.model_base.hyperparameter_tune(X=X_fold, y_train=y_train_fold, X_val=X_val_fold, y_val=y_val_fold, scheduler_options=scheduler_options, **kwargs)
