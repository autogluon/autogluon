...
def model_trial(args, reporter: LocalStatusReporter):
    ...
    try:
        ...
        X, y_train = load_pkl.load(util_args.directory + util_args.dataset_train_filename)
        X_val, y_val = load_pkl.load(util_args.directory + util_args.dataset_val_filename)

        fit_model_args = dict(X=X, y_train=y_train, X_val=X_val, y_val=y_val, **util_args.get('fit_kwargs', dict()))
...
def skip_hpo(model, X, y_train, X_val, y_val, scheduler_options=None, **kwargs):
    """Skips HPO and simply trains the model once with the provided HPO time budget. Returns model artifacts as if from HPO."""
    fit_model_args = dict(X=X, y_train=y_train, **kwargs)
    predict_proba_args = dict(X=X_val)
