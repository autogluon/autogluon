...
class AbstractModel:
    ...
    def __init__(self, path: str = None, name: str = None, problem_type: str = None, eval_metric: Union[str, metrics.Scorer] = None, hyperparameters=None):
        ...
        # Info required for stack info leak protection
        self._model_needs_protection = None  # Whether the model needs to protection or is robust against the leak
        self._reasonable_ensemble = None  # Weights for protection method
        self._leak_protection_for_fit_single = None  # Whether the OOF is generated by one model (e.g. OOB from RF) or via cross-validation.
        self._flip_normalization = None  # Whether we want to normalize flipped values
    ...
    def _post_fit(self, **kwargs):
        ...
        if self.params_aux.get("reasonable_ensemble", False):  # TODO: get a flag depending on l1 or l2+ model.
            self._model_needs_protection = True  # TODO: add option to disable protection based on model args
            self._leak_protection_for_fit_single = kwargs.get("fit_single", False)
            _reasonable_ensemble = self.params_aux["reasonable_ensemble"]

            if self._model_needs_protection:
                if self._leak_protection_for_fit_single:
                    self._reasonable_ensemble = _reasonable_ensemble
                else:
                    self._reasonable_ensemble = [fm for fm in _reasonable_ensemble.models if fm.name == self.name][0]

                self._flip_normalization = self.params_aux.get("stack_info_leak_protection_protection_normalization", False)

                # No support for multiclass for now
                if self.problem_type in [MULTICLASS]:
                    self._flip_normalization = False
    ...
    def _apply_leak_protection(
        ...
        def _get_masks(labels):
            ...
            all_cases = abs_diff**2 >= np.full_like(reasonable_loss, _gap) * trim_mean(abs_diff, 0.25) * np.max([reasonable_loss, loss], axis=0)
            return all_cases
