...
class AutoMLFeatureGenerator(AbstractFeatureGenerator):
    ...
    def _compute_feature_transformations(self):
        ...
        if self.enable_categorical_features:
            ...
            if 'nlp_text' in self.feature_type_family:
                self.feature_transformations['category'] += self.feature_type_family['nlp_text']

        if 'nlp_text' in self.feature_type_family:
            nlp_text_features = self.feature_type_family['nlp_text']
            if self.enable_nlp_ratio_features:
                self.feature_transformations['nlp_special'] += nlp_text_features
            if self.enable_nlp_features:
                self.feature_transformations['nlp_ngram'] += nlp_text_features
        ...
        if self.enable_raw_features:
            for type_family in self.feature_type_family:
                if type_family not in ['object', 'nlp_text', 'datetime']:
    ...
    def generate_features(self, X: DataFrame):
        ...
        if self.feature_transformations['nlp_special']:
            for nlp_feature in self.feature_transformations['nlp_special']:
        ...
        if self.feature_transformations['category']:
            ...
            if 'nlp_text' in self.feature_type_family:
                self.feature_type_family_generated['nlp_category'] += self.feature_type_family['nlp_text']
        ...
        if self.feature_transformations['nlp_special']:
            if not self.fit:
                ...
                self.feature_type_family_generated['nlp_special'] += list(X_text_features_combined.columns)
        ...
        if self.feature_transformations['nlp_ngram']:
            # Combine Text Fields
            features_nlp_current = ['__nlp__']

            if not self.fit:
                features_nlp_to_remove = []
                logger.log(15, 'Fitting vectorizer for nlp features: ' + str(self.feature_transformations['nlp_ngram']))
                for nlp_feature in features_nlp_current:
                    # TODO: Preprocess text?
                    if nlp_feature == '__nlp__':
                        text_list = list(set(['. '.join(row) for row in X[self.feature_transformations['nlp_ngram']].values]))
                    else:
                    ...
                    try:
                        ...
                        features_nlp_to_remove = self.feature_transformations['nlp_ngram']

                self.feature_transformations['nlp_ngram'] = [feature for feature in self.feature_transformations['nlp_ngram'] if feature not in features_nlp_to_remove]
            ...
            while keep_trying_nlp:
                try:
                    ...
                    if not self.fit:
                        self.feature_type_family_generated['nlp_ngram'] += list(X_nlp_features_combined.columns)
                    ...
                    if skip_nlp:
                        ...
                        self.vectorizers = []
                        if 'nlp_ngram' in self.feature_transformations:
                            self.feature_transformations.pop('nlp_ngram')
                        if 'nlp_ngram' in self.feature_type_family_generated:
                            self.feature_type_family_generated.pop('nlp_ngram')
    ...
    def generate_nlp_ngrams(self, X, features_nlp_current, downsample_ratio: int = None):
        ...
        for i, nlp_feature in enumerate(features_nlp_current):
            ...
            if nlp_feature == '__nlp__':
                text_data = ['. '.join(row) for row in X[self.feature_transformations['nlp_ngram']].values]
            else:
