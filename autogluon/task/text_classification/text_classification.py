...
class TextClassification(BaseTask):
    ...
    def fit(dataset='SST',
        ...
        use_roberta = results.pop('use_roberta')
        model = get_network(bert, results.pop('class_labels'), use_roberta)
        update_params(model, results.pop('model_params'))
        transform = results.pop('transform')
        test_transform = results.pop('test_transform')
        return TextClassificationPredictor(model, use_roberta, transform, test_transform, results, checkpoint, args)
