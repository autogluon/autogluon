...
__all__ = ['enas_unit', 'enas_net', 'Zero_Unit', 'ENAS_Unit',
           'ENAS_Sequential']

class ENAS_Unit(torch.nn.Module):
    ...
    @property
    def nparams(self):
        nparams = sum(p.numel() for p in self.module_list[self.index].parameters() if p.requires_grad)
        return nparams
    ...
    def registered_class(Cls):
        class ENAS_Net(Cls):
            def __init__(self, *args, **kwargs):
                ...
                self.enas_modules = torch.nn.ModuleDict(_modules)
                self.latency_evaluated = False
                self._avg_latency = 1
            ...
            def sample(self, **configs):
                ...
                for k in striped_keys:
                    if isinstance(self.enas_modules[k], ENAS_Unit):
                        self.enas_modules[k].sample(configs[k])
                    else:
                        sub_configs = _strip_config_space(configs, prefix=k)
                        self.enas_modules[k].sample(**sub_configs)

            @property
            def nparams(self):
                nparams = 0
                for k, op in self.enas_modules.items():
                    if isinstance(op, (ENAS_Unit, ENAS_Sequential)):
                        nparams += op.nparams
                    else:
                        # standard block
                        nparams += sum(p.numel() for p in op.parameters() if p.requires_grad)
                return nparams

            @property
            def nodeend(self):
                return list(self.enas_modules.keys())[-1]

            @property
            def nodehead(self):
                return list(self.enas_modules.keys())[0]
            ...
            def __repr__(self):
                return 'ENAS_Net -- ' + Cls.__name__

        return ENAS_Net
    return registered_class

class ENAS_Sequential(torch.nn.Module):
    ...
    def __getitem__(self, i):
        return self.enas_modules[str(i)]
    ...
    def graph(self):
        ...
        e = Digraph(node_attr={'color': 'lightblue2', 'style': 'filled', 'shape': 'box'})
        pre_node = None
        for i, op in self.enas_modules.items():
            if hasattr(op, 'graph'):
                e.subgraph(op.graph)
                if pre_node:
                    e.edge(pre_node, op.nodehead)
...
class Zero_Unit(torch.nn.Module):
    def forward(self, x):
        return x
