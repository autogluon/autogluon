...
class CloudPredictor:
    ...
    def __init__(
        ...
        cloud_output_path: str
            Path to s3 location where trained models and intermediate outputs should be should be saved
            If None, CloudPredictor will create one.
        verbosity: int
            Verbosity levels range from 0 to 4 to control how much information is displayed
        """
        ...
        self.cloud_output_path = self._setup_cloud_outputpath(cloud_output_path)
        self.predictor_type = predictor_type
        self.endpoint = None
        self.cloud_predictor_path = None
        self.recent_predict_results_path = None
        self.transformer = None
        self.fit_job_name = None
        self.transform_job_name = None
    ...
    def info(self):
        ...
        """
        info = dict(
            local_output_path=self.path,
            cloud_output_path=self.cloud_output_path,
            predictor_type=self.predictor_type,
            recent_fit_job=dict(
                name=self.fit_job_name,
                status=self.get_fit_job_status(self.fit_job_name),
            ),
            recent_transform_job=dict(
                name=self.transform_job_name,
                status=self.get_batch_transform_job_status(self.transform_job_name),
                result_path=self.recent_predict_results_path,
            ),
            endpoint=self.endpoint_name
        )
        return info

    def _setup_local_outputpath(self, path):
        if path is None:
    ...
    def _setup_cloud_outputpath(self, path):
        if path is None:
    ...
    def _construct_config(self, predictor_cls, predictor_init_args, predictor_fit_args, leaderboard):
        assert predictor_cls in ['tabular', 'text'], f'{predictor_cls} is not yet supported'
        config = dict(
        ...
        path = os.path.join(self.path, 'utils', 'config.yaml')
    ...
    def fit(
        ...
        **kwargs:
        ...
        config = self._construct_config(self.predictor_type, predictor_init_args, predictor_fit_args, leaderboard)
        ...
        try:
            ...
            self.cloud_predictor_path = sagemaker_estimator.model_data
    ...
    def download_trained_predictor(self, save_path=None):
        ...
        """
        assert self.cloud_predictor_path, 'No cloud trained model found.'
        path = self.cloud_predictor_path
        if not save_path:
            save_path = self.path
    ...
    def to_local_predictor(self, save_path=None):
        ...
        """
        assert self.predictor_type in ['tabular', 'text']
        if self.predictor_type == 'tabular':
            from autogluon.tabular import TabularPredictor
            predictor_type = TabularPredictor
        elif self.predictor_type == 'text':
            from autogluon.text import TextPredictor
            predictor_type = TextPredictor
        local_model_path = self.download_trained_predictor(save_path)
        return predictor_type.load(local_model_path)
    ...
    def deploy(
        ...
        **kwargs:
        ...
        if not predictor_path:
            assert self.cloud_predictor_path, 'No cloud trained model found.'
            predictor_path = self.cloud_predictor_path
    ...
    def predict(
        ...
        **kwargs:
        ...
        if not predictor_path:
            assert self.cloud_predictor_path, 'Pleas provide either a path to a predictor tarball or use CloudPredictor.fit() to train one'
            predictor_path = self.cloud_predictor_path
    ...
    def download_predict_results(self, save_path=None):
        ...
        save_path: str
            ...
            If None, CloudPredictor will create one.
        """
        assert self.get_batch_transform_job_status(self.transform_job_name) == 'Completed', 'Transform job not completed yet'
        assert self.recent_predict_results_path, 'No predict results found.'
        file_name = self.recent_predict_results_path.split('/')[-1]
        if not save_path:
            save_path = self.path
    ...
    def save(self, silent=False):
        ...
        if self.endpoint:
            ...
            self.endpoint = None
            self.endpoint_saved = self.endpoint_name
        ...
        if temp_endpoint:
            ...
            self.endpoint_saved = None
        if not silent:
            logger.log(20, f'CloudPredictor saved. To load, use: predictor = CloudPredictor.load("{self.path}")')
