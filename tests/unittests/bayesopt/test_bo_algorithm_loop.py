...
def test_canonical_loop_bo_mcmc(batch_size, seed, global_minimum):
    mx.random.seed(seed)
    np.random.seed(seed)
    random.seed(seed)
    ...
    def _get_next_candidates_with_bo():
        state = _get_current_optim_state()
        gpmodel = default_gpmodel_mcmc(state, 0, DEFAULT_MCMC_CONFIG)
        model = GPMXNetModel(
            state, active_metric, 0, gpmodel, fit_parameters=True,
            num_fantasy_samples=20)
        done_candidates = [candidate_evaluation.candidate for candidate_evaluation in candidate_evaluations]
    ...
    # is the next candidate suggested reproducible?
    A = _get_next_candidates_with_bo()
    B = _get_next_candidates_with_bo()
